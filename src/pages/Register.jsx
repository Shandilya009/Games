...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login(response.user);
      showToast('Account created successfully! Welcome to GameHub!', 'success');
      navigate('/');
    } catch (err) {
    ...
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center p-8 relative overflow-hidden">
      <div className="bg-[rgba(26,35,50,0.95)] backdrop-blur-2xl p-14 rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.5)] w-full max-w-lg border border-[rgba(255,255,255,0.3)] relative z-10 animate-[fadeInUp_0.6s_ease-out]">
        ...
        <p className="text-[#a0aec0] text-center mb-10 text-lg font-normal">
          Create an account to start playing and competing!
        </p>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-7">
          <div className="flex flex-col gap-3">
            <label htmlFor="username" className="font-bold text-white text-sm uppercase tracking-wide">Username</label>
            <input
              type="text"
              id="username"
              name="username"
          ...
          <div className="flex flex-col gap-3">
            <label htmlFor="confirmPassword" className="font-bold text-white text-sm uppercase tracking-wide">Confirm Password</label>
            <input
              type="password"
              id="confirmPassword"
        ...
        <p className="text-center mt-10 text-[#a0aec0] text-base">
          Already have an account? <Link to="/login" className="text-[#00d4ff] font-bold transition-all relative hover:underline">Login here</Link>
        </p>
      </div>
    </div>
  );
